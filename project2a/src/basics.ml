...
let rec factorial x = if x = 1 || x = 0 then 1 else x * factorial (x - 1);;

let rec pow x y = if y = 1 then x else if y = 0 then 1 else x * pow x (y - 1);;
...
let rec rotateHelp x lst a =
  match lst with
  | [] -> lst
  | h::t -> if x = 0 then h::(combine t (reverse a)) else rotateHelp (x - 1) t (h::a);;


let rec rotate shift lst = if shift >= (length lst) then rotateHelp (shift - (length lst)) lst []
else rotateHelp shift lst [];;